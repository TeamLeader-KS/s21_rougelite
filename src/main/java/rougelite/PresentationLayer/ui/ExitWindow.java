...
public class ExitWindow {
    ...
    private static final String[] MENU_PARAGRAPH =
    {
        "Новая игра",
        "Сохранить и выйти",
        "Таблица лидеров"
    ...
    private static void drawContent(Screen screen, int startX, int startY, int width) {
        ...
        for (int i = 0; i < MENU_PARAGRAPH.length; i++) {
            String paragraph = MENU_PARAGRAPH[i];
            String menuLine;
            if (i == selectedIndex) {
                menuLine = "⚔ " + paragraph + " ⚔";
            } else
            ...
            TextColor color = (i == selectedIndex) ? TextColor.ANSI.YELLOW_BRIGHT : TextColor.ANSI.YELLOW;
            drawTextCentered(screen, centerX, y + i * 2, menuLine, color);
        }
    }

    public static void nextItem() {
        selectedIndex = (selectedIndex + 1) % MENU_PARAGRAPH.length;
    }

    public static void prevItem() {
        selectedIndex = (selectedIndex - 1 + MENU_PARAGRAPH.length) % MENU_PARAGRAPH.length;
    }
    ...
    public static void setDefaultSelection() {
        selectedIndex = 1; // всегда "Сохранить и выйти"
    }
